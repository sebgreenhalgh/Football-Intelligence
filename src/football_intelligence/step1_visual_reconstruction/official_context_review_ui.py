...
from football_intelligence.step1_visual_reconstruction.io import (
    STEP1D1B_CANDIDATE_CONTEXT_IMAGES_DIR,
    STEP1D1B_CANDIDATE_CROP_IMAGES_DIR,
    STEP1D1B_CANDIDATE_FULL_FRAME_IMAGES_DIR,
    STEP1D1B_MANUAL_REVIEW_UI_HTML_PATH,
    STEP1D1B_OUTPUT_DIR,
...
from football_intelligence.step1_visual_reconstruction.official_context_review_eval import (
...
from football_intelligence.step1_visual_reconstruction.official_context_review_state import review_state_payload
...
DECISION_BUTTONS = [
    ("Enter", "Accept D1", "accept_d1_belief"),
    ("r", "Official/referee", "correct_to_official_referee_like"),
    ("l", "Assistant/line", "correct_to_assistant_or_line_official_like"),
    ("c", "Context person", "correct_to_non_official_context_person_like"),
    ("o", "Off-pitch context", "correct_to_off_pitch_context_person_like"),
    ("p", "Player-like", "correct_to_player_like_not_official_context"),
    ("b", "Bad / not person", "correct_to_bad_detection_or_not_person"),
    ("u", "Unknown context", "correct_to_unknown_official_context"),
...
def require_cv2() -> Any:
    if cv2 is None or np is None:
        raise RuntimeError("OpenCV and NumPy are required for Step1.D1b review image generation. Use the project venv interpreter.")
    return cv2


def safe_asset_stem(value: str) -> str:
    return SAFE_NAME_RE.sub("_", str(value).strip()) or "unknown_d1b_candidate"


def rel_asset_path(path: Path) -> str:
    return path.resolve().relative_to(STEP1D1B_OUTPUT_DIR.resolve()).as_posix()
...
def padded_bbox(row: dict[str, Any], shape: tuple[int, int], pad_fraction: float = 1.15) -> dict[str, int] | None:
...
def draw_full_frame(image: Any, row: dict[str, Any], width: int = 560) -> Any:
    cv2_module = require_cv2()
...
def asset_paths(candidate: dict[str, Any]) -> dict[str, Path]:
    stem = safe_asset_stem(str(candidate.get("step1d1_review_candidate_id", "")))
    return {
        "crop_image": STEP1D1B_CANDIDATE_CROP_IMAGES_DIR / f"{stem}_crop.jpg",
        "context_image": STEP1D1B_CANDIDATE_CONTEXT_IMAGES_DIR / f"{stem}_context.jpg",
        "full_frame_image": STEP1D1B_CANDIDATE_FULL_FRAME_IMAGES_DIR / f"{stem}_full_frame.jpg",
    }
...
def write_image(path: Path, image: Any) -> None:
    ...
    if not cv2_module.imwrite(str(path), image):
        raise RuntimeError(f"OpenCV could not write Step1.D1b image: {path}")
...
def generate_candidate_assets(candidates: list[dict[str, Any]] | None = None) -> dict[str, dict[str, str]]:
    ...
    ensure_dir(STEP1D1B_CANDIDATE_CROP_IMAGES_DIR)
    ensure_dir(STEP1D1B_CANDIDATE_CONTEXT_IMAGES_DIR)
    ensure_dir(STEP1D1B_CANDIDATE_FULL_FRAME_IMAGES_DIR)
    ...
    for candidate in candidates:
        ...
        if image is None:
            ...
            full = placeholder((560, 150), f"missing frame {seq}")
        else:
        ...
        assets[str(candidate.get("step1d1_review_candidate_id", ""))] = {key: rel_asset_path(path) for key, path in paths.items()}
    return assets


def default_assets_for_candidates(candidates: list[dict[str, Any]]) -> dict[str, dict[str, str]]:
    return {
        str(candidate.get("step1d1_review_candidate_id", "")): {
...
<head>
  ...
  <title>Step1.D1b Official Context Review</title>
  <style>
    :root { --bg:#f4f3ef; --ink:#151719; --muted:#646b73; --line:#cfd4d8; --panel:#fff; --soft:#eef2f4; --ok:#0f766e; --warn:#9a5b00; }
    * { box-sizing:border-box; }
    ...
    main { max-width:1720px; margin:0 auto; padding:14px; display:grid; grid-template-columns:minmax(340px,1.35fr) minmax(330px,.75fr); gap:14px; }
    .media-grid { display:grid; grid-template-columns:minmax(260px,.82fr) minmax(300px,1.18fr); gap:12px; align-items:start; }
    ...
    .full img { max-height:200px; }
    .meta { padding:10px; display:grid; grid-template-columns:minmax(120px,.44fr) minmax(170px,.56fr); gap:6px 12px; font-size:13px; }
    .k { color:var(--muted); }
...
<body>
  <header>
    <div><h1>Step1.D1b Official Context Review</h1><div class="subline"><span id="position">0 / 0</span><span id="candidateId"></span><span>VISUAL_ONLY_NOT_METRIC</span></div></div>
    <div class="pills" id="pills"></div>
  </header>
  <main>
    <div class="media-grid">
      <section><h2>Torso Crop</h2><div class="image-wrap"><img id="cropImage" alt=""></div></section>
      <div class="side">
        <section><h2>Context Crop</h2><div class="image-wrap"><img id="contextImage" alt=""></div></section>
        <section class="full"><h2>Full Frame</h2><div class="image-wrap"><img id="fullImage" alt=""></div></section>
      </div>
    </div>
    <div class="side">
  ...
  <script>window.STEP1D1B_BOOTSTRAP=__BOOTSTRAP_JSON__;</script>
  <script>
    ...
    function renderPills(){const p=app.progress||{},d=app.decisionSummary||{}; const ok=d.d1b_approve_d1_for_next_stage_candidate; els.pills.innerHTML=[`<span class="pill">reviewed ${p.reviewed_candidates||0} / ${p.total_review_candidates||0}</span>`,`<span class="pill">accepted ${p.accepted_d1_count||0}</span>`,`<span class="pill">corrected ${p.corrected_count||0}</span>`,`<span class="pill">unsure ${p.unsure_count||0}</span>`,`<span class="pill ${ok?'good':'wait'}">${ok?'gate pass':'gate pending'}</span>`].join("")}
    function metaItems(row){return [["review_reason",row.review_reason],["frame_sequence",row.frame_sequence],["gold_proxy_tag",row.gold_proxy_tag],["D1 belief",`${row.official_context_belief} ${row.official_context_belief_confidence}`],["D1 state",row.official_context_belief_state],["D1 reason",row.official_context_belief_reason],["reason tags",(row.review_reason_tags||[]).join(", ")],["warning flags",(row.official_context_warning_flags||[]).join(", ")],["C2c colour",row.c2c_final_colour_belief],["C2c source",row.c2c_colour_source],["C2c human reviewed",row.c2c_human_reviewed],["candidate_type",row.candidate_type],["original_role_source",row.original_role_source],["roi_status",row.roi_status],["source official/player/context",`${row.source_official_candidate_flag}/${row.source_player_candidate_flag}/${row.source_unknown_context_candidate_flag}`],["saved decision",row.saved_human_review_decision||""],["saved corrected",row.saved_human_corrected_official_context_belief||""]]}
    ...
    function render(){renderPills();renderButtons();const row=current(); if(!row){els.position.textContent="0 / 0"; return} els.position.textContent=`${app.index+1} / ${app.rows.length}`; els.candidateId.textContent=row.step1d1_review_candidate_id; const a=row.ui_assets||{}; els.crop.src=a.crop_image||""; els.context.src=a.context_image||""; els.full.src=a.full_frame_image||""; els.meta.innerHTML=metaItems(row).map(([k,v])=>`<div class="k">${k}</div><div class="v">${v??""}</div>`).join(""); els.notes.value=row.saved_reviewer_notes||""; app.confidence=row.saved_human_review_confidence||app.confidence||"high"; renderButtons()}
    ...
    async function saveDecision(decision){const row=current(); if(!row)return; if(app.readonly){setStatus("Open via the local server to autosave."); return} setStatus("Saving..."); const response=await fetch("/api/review",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({step1d1_review_candidate_id:row.step1d1_review_candidate_id,human_review_decision:decision,human_review_confidence:app.confidence||"high",reviewer_name:els.reviewer.value,reviewer_notes:els.notes.value})}); if(!response.ok){setStatus(`Save failed: ${response.status}`); return} const payload=await response.json(); app.progress=payload.progress; app.decisionSummary=payload.decision_summary; const idx=app.rows.findIndex(r=>r.step1d1_review_candidate_id===payload.updated_candidate.step1d1_review_candidate_id); if(idx>=0)app.rows[idx]=payload.updated_candidate; setStatus(`Saved ${decision}`); render(); skip()}
...
def review_ui_manifest_payload(assets_by_id: dict[str, dict[str, str]], *, host: str = "127.0.0.1", port: int = 8776) -> dict[str, Any]:
    candidates = review_state_payload().get("rows", [])
    decision = review_decision_summary_payload(candidates, load_reviewed_decisions())
    return {
        "artifact": "step1d1b_review_ui_manifest",
        "created_at": utc_iso(),
        "match_id": MATCH_ID,
        "clip_id": CLIP_ID,
        ...
        "auto_promoted": False,
        "launch": {
            "serve_command": f".\\.venv\\Scripts\\python.exe scripts\\step1d1b_launch_official_context_review_ui.py --serve --port {port}",
            "url": f"http://{host}:{port}/",
            "static_html_path": str(STEP1D1B_MANUAL_REVIEW_UI_HTML_PATH.resolve()),
        ...
        "outputs": {
            ...
            "step1d1b_manual_review_ui_html_path": str(STEP1D1B_MANUAL_REVIEW_UI_HTML_PATH.resolve()),
            "step1d1b_candidate_crop_images_dir": str(STEP1D1B_CANDIDATE_CROP_IMAGES_DIR.resolve()),
            "step1d1b_candidate_context_images_dir": str(STEP1D1B_CANDIDATE_CONTEXT_IMAGES_DIR.resolve()),
            "step1d1b_candidate_full_frame_images_dir": str(STEP1D1B_CANDIDATE_FULL_FRAME_IMAGES_DIR.resolve()),
        },
        "summary": {
            ...
            "d1b_approve_d1_for_next_stage_candidate": decision.get("d1b_approve_d1_for_next_stage_candidate", False),
        },
    }


def prepare_official_context_review_ui(*, host: str = "127.0.0.1", port: int = 8776) -> dict[str, Any]:
    ensure_dir(STEP1D1B_OUTPUT_DIR)
    export_existing_reviewed_decisions()
    state = review_state_payload()
    assets = generate_candidate_assets(state.get("rows", []))
    state = merged_state(assets)
    write_text(STEP1D1B_MANUAL_REVIEW_UI_HTML_PATH, render_static_html(state))
    manifest = review_ui_manifest_payload(assets, host=host, port=port)
    write_json(STEP1D1B_REVIEW_UI_MANIFEST_PATH, manifest)
    return manifest
...
def resolve_asset_path(raw_path: str) -> Path | None:
    ...
    candidate = (STEP1D1B_OUTPUT_DIR / rel).resolve()
    if not candidate.is_relative_to(STEP1D1B_OUTPUT_DIR.resolve()):
        return None
...
class OfficialContextReviewHandler(BaseHTTPRequestHandler):
    server_version = "Step1D1bOfficialContextReview/1.0"
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path in {"/", "/index.html", "/step1d1b_manual_review_ui.html"}:
            text_response(self, 200, STEP1D1B_MANUAL_REVIEW_UI_HTML_PATH.read_text(encoding="utf-8"))
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            save_single_review_decision(
                str(payload.get("step1d1_review_candidate_id", "")),
            ...
            state = merged_state()
            updated = next((row for row in state["rows"] if row.get("step1d1_review_candidate_id") == payload.get("step1d1_review_candidate_id")), {})
...
def serve_official_context_review_ui(*, host: str = "127.0.0.1", port: int = 8776) -> None:
    prepare_official_context_review_ui(host=host, port=port)
    server = ThreadingHTTPServer((host, port), OfficialContextReviewHandler)
    print(f"Step1.D1b official/context review UI: http://{host}:{port}/")
    print(f"Reviewed decisions autosave path: {STEP1D1B_REVIEWED_DECISIONS_PATH.resolve()}")
    server.serve_forever()


def print_step1d1b_ui_console(manifest: dict[str, Any]) -> None:
    outputs = manifest["outputs"]
    summary = manifest["summary"]
    decision = read_json(STEP1D1B_REVIEW_DECISION_SUMMARY_PATH)

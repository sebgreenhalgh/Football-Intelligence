...
from football_intelligence.step1_visual_reconstruction.official_context_beliefs import ALLOWED_OFFICIAL_CONTEXT_BELIEFS
...
DECISION_TO_CORRECTED_BELIEF = {
    "accept_d1_belief": "",
    "correct_to_official_referee_like": "official_referee_like",
    "correct_to_assistant_or_line_official_like": "assistant_or_line_official_like",
    "correct_to_non_official_context_person_like": "non_official_context_person_like",
    "correct_to_off_pitch_context_person_like": "off_pitch_context_person_like",
    "correct_to_player_like_not_official_context": "player_like_not_official_context",
    "correct_to_bad_detection_or_not_person": "bad_detection_or_not_person",
    "correct_to_unknown_official_context": "unknown_official_context",
    "unsure_needs_later_review": "unknown_official_context",
}

ALLOWED_REVIEW_CONFIDENCES = {"high", "medium", "low"}
D1B_FORBIDDEN_KEYS = set(FORBIDDEN_OUTPUT_KEYS) | {
    "track_id",
...
def corrected_belief_for_decision(decision: str, d1_belief: str) -> str:
    if decision not in ALLOWED_HUMAN_OFFICIAL_CONTEXT_DECISIONS:
...
def reviewed_decision_row(
    ...
    *,
    ...
    d1_belief = str(candidate.get("official_context_belief", "unknown_official_context"))
    corrected = corrected_belief_for_decision(human_review_decision, d1_belief)
    ...
    return {
        "step1d1_review_candidate_id": candidate.get("step1d1_review_candidate_id", ""),
        ...
        "d1_official_context_belief_confidence": safe_float(candidate.get("official_context_belief_confidence")),
        "d1_official_context_belief_state": candidate.get("official_context_belief_state", ""),
        "d1_review_reason_tags": candidate.get("review_reason_tags", []),
        "human_review_decision": human_review_decision,
        "human_corrected_official_context_belief": corrected,
...
def validate_reviewed_decision_payload(
    ...
    *,
    ...
    for index, row in enumerate(rows):
        candidate_id = str(row.get("step1d1_review_candidate_id", ""))
        candidate = known.get(candidate_id)
        if not candidate:
            errors.append({"row_index": index, "step1d1_review_candidate_id": candidate_id, "error": "unknown_review_candidate_id"})
            continue
        forbidden = sorted(key for key in D1B_FORBIDDEN_KEYS if key in row)
        if forbidden:
            errors.append({"row_index": index, "step1d1_review_candidate_id": candidate_id, "error": "forbidden_keys_present", "keys": forbidden})
        if row.get("production_ready") is True:
            errors.append({"row_index": index, "step1d1_review_candidate_id": candidate_id, "error": "production_ready_true_rejected"})
        decision = str(row.get("human_review_decision", ""))
        if decision not in ALLOWED_HUMAN_OFFICIAL_CONTEXT_DECISIONS:
            errors.append({"row_index": index, "step1d1_review_candidate_id": candidate_id, "error": "human_review_decision_not_allowed", "decision": decision})
        confidence = str(row.get("human_review_confidence", ""))
        if confidence and confidence not in ALLOWED_REVIEW_CONFIDENCES:
            errors.append({"row_index": index, "step1d1_review_candidate_id": candidate_id, "error": "human_review_confidence_not_allowed", "confidence": confidence})
        corrected = str(row.get("human_corrected_official_context_belief", ""))
        if decision in ALLOWED_HUMAN_OFFICIAL_CONTEXT_DECISIONS:
            expected = corrected_belief_for_decision(decision, str(candidate.get("official_context_belief", "")))
            if corrected != expected:
                errors.append({"row_index": index, "step1d1_review_candidate_id": candidate_id, "error": "human_corrected_belief_does_not_match_decision", "expected": expected, "actual": corrected})
        if corrected not in ALLOWED_OFFICIAL_CONTEXT_BELIEFS:
            errors.append({"row_index": index, "step1d1_review_candidate_id": candidate_id, "error": "human_corrected_belief_not_allowed", "belief": corrected})

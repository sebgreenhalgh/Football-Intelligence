...
from football_intelligence.replay.differential import media_diff  # noqa: E402
...
def test_m4_replay_media_parity(tmp_path: Path) -> None:
    replay = tmp_path / "reconstructed_m4"
    shutil.copytree(PRESERVED_M4, replay)
    diff = media_diff(PRESERVED_M4, replay)
    assert diff["passed"] is True
    assert diff["asset_count"] == 857

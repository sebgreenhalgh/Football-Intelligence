...
def test_no_forbidden_keys_in_d1c_rows_or_template() -> None:
    row = build_d1c_row(d1_row(), None)
    template = review_decision_template_payload()
    assert not (set(row) & D1C_FORBIDDEN_KEYS)
    assert row["production_ready"] is False
    assert row["eligible_for_identity_tracking"] is False
    assert row["eligible_for_player_slot_assignment"] is False
    assert template["approve_any_official_exclusion"] is False
    assert template["approve_any_player_slot_use"] is False
    assert template["production_ready"] is False


def test_no_stage3d_registry_paths_or_stage3c_promotion_imports_in_d1c_sources() -> None:
    source_paths = [
        SRC / "official_context_human_corrections.py",
        SRC / "official_context_correction_eval.py",
        SRC / "official_context_correction_render.py",
        ROOT / "scripts" / "step1d1c_apply_human_official_context_corrections.py",
        ROOT / "scripts" / "step1d1c_evaluate_human_corrected_official_context.py",
        ROOT / "scripts" / "step1d1c_render_human_corrected_official_context_review.py",
        ROOT / "scripts" / "step1d1c_build_review_pack.py",
    ]

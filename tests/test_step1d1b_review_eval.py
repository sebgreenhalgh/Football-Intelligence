...
def reviewed(rows: list[dict]) -> dict[str, dict]:
    return {
        row["step1d1_review_candidate_id"]: reviewed_decision_row(row, "accept_d1_belief")
        for row in rows
    }
...
def gate_pass_candidates() -> list[dict]:
    ...
    rows.extend(
        candidate(400 + index, official_context_belief="bad_detection_or_not_person")
...
def test_save_single_review_decision_writes_autosave_payload(tmp_path: Path) -> None:
    first = ordered_review_candidates()[0]

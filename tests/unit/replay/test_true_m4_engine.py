...
def test_true_m4_engine_builds_rows_and_assets_without_baseline(tmp_path: Path) -> None:
    ...
    assert result["reads_preserved_m4_content"] is False
    assert result["counts"]["m4_handoff_pathlet_count"] == 1
    assert (run_root / "reconstructed_m4/step2m4_sparse_handoff_edges.jsonl.gz").exists()

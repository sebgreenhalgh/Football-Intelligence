...
def repository_values(*, head: str, changed: str = "", count: str = "0") -> dict[tuple[str, ...], str]:
    baseline = reaudit.R2_REVIEWER_COMMIT
    return {
        ("rev-parse", "HEAD"): head,
...
def test_exact_r2_head_and_origin_are_accepted(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr(reaudit, "git", fake_git(repository_values(head=reaudit.R2_REVIEWER_COMMIT)))
    result = reaudit.verify_repository_compatibility(tmp_path, reaudit.R2_REVIEWER_COMMIT)
    assert result["head"] == reaudit.R2_REVIEWER_COMMIT
    assert result["changed_paths_above_r2"] == []
...
def test_unrelated_repository_drift_fails_closed(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="not the exact Phase-B compatibility repair"):
        reaudit.verify_repository_compatibility(tmp_path, reaudit.R2_REVIEWER_COMMIT)


def test_exact_phase_b_compatibility_drift_is_accepted(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    head = "e" * 40
    changed = "\n".join(sorted(reaudit.ALLOWED_COMPATIBILITY_PATHS))
    monkeypatch.setattr(reaudit, "git", fake_git(repository_values(head=head, changed=changed, count="1")))
    result = reaudit.verify_repository_compatibility(tmp_path, reaudit.R2_REVIEWER_COMMIT)
    assert result["commits_above_r2"] == 1
    assert result["changed_paths_above_r2"] == sorted(reaudit.ALLOWED_COMPATIBILITY_PATHS)

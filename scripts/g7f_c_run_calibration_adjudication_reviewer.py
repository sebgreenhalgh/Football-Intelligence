"""Check or serve the six-frame G7F-C calibration adjudication reviewer."""
...
BASELINE = "f14d33b50ae596eee95eaa0b5466034bf40faa1c"
EXPECTED_HASHES = {
...
def roots(repo: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "stage": part9 / "G7F_C_CALIBRATION_SUPERSEDING_ADJUDICATION_AND_REAUDIT_v1",
    }
...
def verify_release(paths: dict[str, Path]) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if not commit or head != commit or origin != commit:
        raise RuntimeError(f"reviewer release requires HEAD == origin/main == {commit}; found {head}, {origin}")
    if release.get("required_baseline") != BASELINE or release.get("reviewer_release") != REVIEWER_RELEASE:
        raise RuntimeError("reviewer release identity or baseline binding mismatch")
    for row in release.get("files", []):
        path = Path(row["path"])
        if not path.is_file() or path.stat().st_size != row["byte_size"] or sha256_file(path) != row["sha256"]:
            raise RuntimeError(f"reviewer release file mismatch: {path}")
    bindings = read_json(stage / "05_REVIEWER/reviewer_binding_hashes.json")
    return release, {str(key): str(value) for key, value in bindings.items()}


def verify_original_inventory(paths: dict[str, Path]) -> dict[str, dict[str, str]]:
    inventory = read_json(paths["stage"] / "01_FREEZE/original_real_decisions_inventory.json")
    root = paths["source"] / "06_DENSE_DECISIONS"
...
def main() -> int:
    ...
    if args.phase == "check":
        ...
        return 0
    print(f"G7F-C calibration adjudication reviewer: http://{config.host}:{config.port}/")

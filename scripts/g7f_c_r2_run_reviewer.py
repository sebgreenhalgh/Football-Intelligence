"""Launch the G7F-C R2 reviewer against the real calibration decision root."""
...
def verify_runtime_and_contracts(all_paths: dict[str, Path]) -> dict[str, str]:
    if sys.version_info[:2] != (3, 12):
        raise RuntimeError(f"G7F-C R2 requires Python 3.12; found {sys.version.split()[0]}")
    ...
    bindings_path = all_paths["repair"] / "05_REVIEWER/reviewer_binding_hashes.json"
    if not bindings_path.is_file():
        raise RuntimeError(f"R2 release bindings are missing: {bindings_path}")
    ...
    for key, value in expected_binding_values.items():
        if bindings.get(key) != value:
            raise RuntimeError(f"R2 binding mismatch for {key}")
    return bindings


def reviewer_config(repo: Path, *, port: int = 8791) -> DensePersonReviewerConfig:
    all_paths = roots(repo)
    bindings = verify_runtime_and_contracts(all_paths)
    source = all_paths["source"]
...
def main() -> int:
    ...
    if args.phase == "check":
        ...
        return 0
    print(f"G7F-C R2 calibration reviewer: http://{config.host}:{config.port}/")
